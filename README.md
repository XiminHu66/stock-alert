...
完整数据由 GitHub Actions 中的 `yfinance` 获取。轻量报价任务直接读取 Yahoo Finance 图表端点，省去每五分钟安装 pandas/yfinance 的时间，并与完整模型任务使用独立队列。每次报价提交也会重新部署 Pages，网页优先读取同域文件以避开 `raw.githubusercontent.com` 的 CDN 陈旧缓存。页面打开时还会每分钟尝试直连刷新当前选中标的；若浏览器或 Yahoo 限制跨域/频率，本次会话会自动停用直连并回退到 GitHub Actions 快照。顶部会分别显示实际行情时间和数据路径。新闻、期权和完整历史数据目标频率约 30 分钟。GitHub 的计划任务属于 best-effort，仍可能因平台调度繁忙而延迟。模型在浏览器中计算，每次成功报价都会重新推演。期权数据是成交量、未平仓量和隐含波动率快照，不是逐笔成交方向或付费订单流。
